...
public class UserService {
  ...
  @Transactional // Part of scheduling jobs
  public UserResponse create(@Valid CreateUserRequest request) {
    User user = new User(request);
    Portfolio defaultPortfolio = new Portfolio(DefaultValues.DEFAULT_PORTFOLIO_NAME);
    Watchlist defaultWatchlist = new Watchlist(DefaultValues.DEFAULT_WATCHLIST_NAME);

    user.addPortfolio(defaultPortfolio);
    user.addWatchlist(defaultWatchlist);
    user = userRepository.save(user);
    sendVerificationEmail(user); // Email will only sent if user is saved

    return new UserResponse(user); // Key Takeaway
                                   // The job is scheduled when you call
                                   // BackgroundJobRequest.enqueue(sendWelcomEmailJob), but it only actually runs
                                   // after the database transaction successfully commits — ensuring consistency
                                   // (email is never sent for a user that failed to save)
  }
